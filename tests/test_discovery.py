...
class TestDiscoveryEngine:
    ...
    def test_discovered_service_fields(self):
        s = DiscoveredService(name="test", description="desc", mcp_endpoint="stdio://test",
                              health_endpoint="http://localhost:8000/health", port=8000,
                              tags=["a", "b"], source="test", confidence=0.95)
        assert s.name == "test"
