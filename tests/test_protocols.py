...
class TestDispatch:
    ...
    async def test_dispatch_mcp_ssrf_blocked(self):
        ...
        result = await dispatch(
            {"protocol": "mcp", "mcp_endpoint": "http://10.0.0.1/mcp"},
            "test.tool", {})
        assert result["status"] == "error"
    ...
    async def test_dispatch_rest_ssrf_blocked(self):
        ...
        result = await dispatch(
            {"protocol": "rest", "mcp_endpoint": "http://10.0.0.1/api",
             "protocol_config": {"method": "GET"}},
            "test.list", {})
        assert result["status"] == "error"
        assert "Service unavailable" in result["error"]
    ...
    async def test_dispatch_grpc_no_stub(self, monkeypatch):
        ...
        result = await dispatch(
            {"protocol": "grpc", "mcp_endpoint": "grpc://192.0.2.1:50051",
             "protocol_config": {"host": "192.0.2.1:50051"}},
            "test.call", {})
        assert result["status"] == "error"
    ...
    async def test_dispatch_ws_invalid_url(self):
        ...
        result = await dispatch(
            {"protocol": "websocket", "mcp_endpoint": "http://192.0.2.1:8080",
             "protocol_config": {"timeout": 1}},
            "test.push", {})
        assert result["status"] == "error"
    ...
    async def test_dispatch_rest_method_post(self, monkeypatch):
        ...
        result = await dispatch(
            {"protocol": "rest", "mcp_endpoint": "http://192.0.2.1:3000/api",
