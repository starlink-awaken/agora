...
class TestAgoraIntegration:
    ...
    def test_register_all_services(self):
        ...
        self.registry.register(Service(
            "sophia", "Paradigm Engine",
            mcp_endpoint="sophia-mcp", port=9001,
            tags=["paradigm", "compiler"],
        ))
        self.registry.register(Service(
            "kos", "Knowledge OS",
            mcp_endpoint="kos-mcp-server", port=9002,
    ...
    def test_full_route_flow(self):
        ...
        # Resolve
        tools = ["minerva.research_now", "minerva.knowledge_search", "minerva.knowledge_ingest",
                 "sophia.compile_paradigm", "sophia.list_operations"]
        for tool in tools:
