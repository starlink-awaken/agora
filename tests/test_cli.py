...
class TestParserStructure:
    ...
    def test_all_top_level_commands(self):
        ...
        expected = {
            "register", "list", "discover", "instance", "tenant", "market",
            "search", "info", "stats", "health", "route", "routes", "mcp",
            "init", "completion", "config", "web", "pipeline", "pipelines",
            "pipeline-define", "event",
        }
        for cmd in expected:
    ...
    def test_register_with_protocol(self):
        parsed = self.parser.parse_args([
            "register", "api", "--protocol", "rest",
            "--protocol-config", '{"method":"GET"}',
        ])
    ...
    def test_register_with_endpoint(self):
        parsed = self.parser.parse_args([
            "register", "minerva", "--mcp", "http://192.0.2.1:8765/mcp",
            "--health", "http://192.0.2.1:8765/health", "--port", "8765",
        ])
    ...
    def test_pipeline_command(self):
        parsed = self.parser.parse_args([
            "pipeline", "my-pipeline", "--goal", "test-goal",
            "--stream", "--parallel",
        ])
