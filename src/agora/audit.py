...
class AuditLogger:
    ...
    def log(self, action: str, actor: str = "anonymous",
            resource: str = "", result: str = "success",
            detail: str = "", ip: str = "") -> str:
        """Record an audit entry. Returns the entry ID."""
        eid = str(uuid.uuid4())[:8]
        ts = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        conn = _get_db(self._db_path)
        conn.execute(
            "INSERT INTO audit_log (id, timestamp, actor, action, resource, result, detail, ip) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (eid, ts, actor, action, resource, result, detail, ip),
        )
        conn.commit()
        return eid

    def query(self, actor: str = "", action: str = "", resource: str = "",
