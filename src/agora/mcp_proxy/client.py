...
class StdioMCPClient(MCPClient):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.error("proxy_subprocess_not_found",
                         service=self.service_name, command=self._command)
            return False
        except Exception as e:
            logger.error("proxy_subprocess_spawn_failed",
        ...
        try:
            ...
            if result:
                ...
                logger.info("proxy_connected", service=self.service_name,
                            transport="stdio")
                return True
        except Exception as e:
            logger.error("proxy_initialize_failed",
                         service=self.service_name, error=str(e))
            await self.disconnect()
            return False
    ...
    async def _read_loop(self):
        ...
        try:
            ...
            logger.error("proxy_read_loop_error",
                         service=self.service_name, error=str(e))
        finally:
            for _req_id, fut in self._pending.items():
                if not fut.done():
...
class HttpMCPClient(MCPClient):
    ...
    async def connect(self) -> bool:
        ...
        if connected:
            self._connected = True
            logger.info("proxy_connected", service=self.service_name,
                        transport="http", endpoint=self._endpoint)
            return True

        logger.warning("proxy_http_probe_failed",
                       service=self.service_name, endpoint=self._endpoint)
        return False
    ...
    async def list_tools(self) -> list[dict]:
        ...
        try:
            ...
            logger.error("proxy_http_list_tools_failed",
                         service=self.service_name, error=str(e))
            return []
...
def create_client(service_name: str, endpoint: str,
                  command: str = "", args: list[str] | None = None,
                  cwd: str | None = None) -> MCPClient:
    ...
    if endpoint.startswith("http"):
        ...
        raise ValueError(f"Cannot create client for {service_name}: "
                         f"endpoint={endpoint!r}, command={command!r}")
