...
from agora.registry import _is_safe_url
...
async def _call_rest(tool_name: str, arguments: dict, instance: dict) -> dict:
    ...
    return {"status": "error",
            "error": f"REST call failed after {max_retries + 1} attempts: "
...
async def _call_grpc(tool_name: str, arguments: dict, instance: dict) -> dict:
    ...
    host = cfg.get("host",
                   endpoint.replace("grpc://", "").rstrip("/"))
    if ":" not in host:
        return {"status": "error",
                "error": f"gRPC host:port required, got: {host}"}
    ...
    try:
        ...
        return {"status": "error",
                "error": f"gRPC call failed: {str(e)[:200]}"}
